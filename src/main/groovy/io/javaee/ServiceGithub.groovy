...
class ServiceGithub {
    public static final String DOCUMENTATION_ROOT = System.getProperty(
            'io_config_root',
            System.getenv()['io_config_root'] ?: 'jcpevangelists'
    )
    public static final String CONFIG_PROJECT = System.getProperty(
            'io_config_project',
            System.getenv()['io_config_project'] ?: 'javaee.io.config'
    )
    ...
    void update() {
        ...
        publishedTagsMap.keySet().each { String projectName ->
            ...
            List<String> tags = parseJsonText(
                    http.getUrlContentWithToken("${HttpBean.BASE_URL}/repos/jcpevangelists/${json.name}/tags")
            ).collect({ it.name })
