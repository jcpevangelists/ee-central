...
You will need a tomitribe github access token and a system property with this value.

[source, bash]
----
export github_atoken=TOKEN_HERE
----
...
maven project. `mvn clean install tomee:run` will do all the heavy lifting for us. It downloads all the dependencies
and executes node.js and gulp for us. It will also start the tomee server at the port 8080.
